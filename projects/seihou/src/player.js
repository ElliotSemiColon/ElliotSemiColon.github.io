import Emitter from "/projects/seihou/src/emitter.js"
export default class Player{ //exports the class for use in game.js
    ...
    constructor(){ //constructs object with these properties 
        ...
        this.streams = 3;
        this.fireRate = 20;
        this.emitter = new Emitter(this.fireRate, 30, 0, this.streams, 0, 10, 0, 0, 10, "rgba(0,0,0)", 10);
        this.smoothedFps = 60;
        this.radius;
    }
        ...
        if (this.lives > 0){
            ...
            if (this.invincible){ //when the player is respawing
                if ((this.spentFrames <= this.invincFrames) && (this.spentFrames % 15 < 10)){
                    ...
                    ctx.arc(this.position.x, this.position.y + ((this.gameWindow.height - this.spawn.y) / Math.pow(this.spentFrames, 0.5) - 15), this.radius, ((2/this.invincFrames) * this.spentFrames) * Math.PI, ((4/this.invincFrames) * this.spentFrames) * Math.PI, false); //does the funky respawn animation
